...
WCHAR szTitleAnswer[MAX_LOADSTRING] = _T("Ответ");
WCHAR szWindowClass[MAX_LOADSTRING] = _T("main");
WCHAR szWindowClassMul[MAX_LOADSTRING] = _T("mul");
WCHAR szWindowClassAnswer[MAX_LOADSTRING] = _T("answer");
...
int AlgrtmDekstr(int A, int B, int FGH);
void saveMatrix();
bool flag = false;
...
ATOM AnswerRegisterClass(HINSTANCE hInstance)//функция регистрации 3 класса
{
	WNDCLASSEXW wcex;

	wcex.cbSize = sizeof(WNDCLASSEX);
	wcex.style = CS_HREDRAW | CS_VREDRAW;
	wcex.lpfnWndProc = AnswerWndProc;
	wcex.cbClsExtra = 0;
	wcex.cbWndExtra = 0;
	wcex.hInstance = hInstance;
	wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_WINAPI));
	...
	wcex.hbrBackground = (HBRUSH)(COLOR_WINDOW);
	wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_WINAPI);
	wcex.lpszClassName = szWindowClassAnswer;
	wcex.hIconSm = LoadIcon(wcex.hInstance, MAKEINTRESOURCE(IDI_SMALL));
...
BOOL InitAnswerInstance(HINSTANCE hInstance, int nCmdShow)//создаём главное окно
{
	Answer = CreateWindowW(szWindowClassAnswer, szTitleAnswer, WS_OVERLAPPEDWINDOW | WS_HSCROLL,
...
LRESULT CALLBACK MainWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)//функция обработки сообщений первого дочернего окна
{
	...
	{
	case WM_CREATE:
		ShowWindow(CreateWindowW(_T("button"), _T("Матрица"), WS_CHILD | WS_VISIBLE,
			20, 20, 60, 20, hWnd, (HMENU)2, nullptr, nullptr), SW_SHOWNORMAL);
		ShowWindow(CreateWindowW(_T("button"), _T("Ответ"), WS_CHILD | WS_VISIBLE,
			90, 20, 60, 20, hWnd, (HMENU)3, nullptr, nullptr), SW_SHOWNORMAL);
		if (!MulRegisterClass(hInst)) return FALSE;
		if (!AnswerRegisterClass(hInst)) return FALSE;
		break;
	case WM_COMMAND:
	{
		...
		case 3: {
			InitAnswerInstance(hInst, SW_SHOWNORMAL);
			break;
		}
		default: {
...
LRESULT CALLBACK AnswerWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)//функция обработки сообщений первого дочернего окна
{
	switch (message)
	{
	case WM_CREATE:
		...
		if (n > 0) {
			...
			for (int i = 1; i < n; i++) {
				for (int j = 1; j < n; j++) {
					wchar_t buffer[256];
					wsprintfW(buffer, L"%d", MassMatrix[i][j]);
					SetWindowTextW(etMatrix2[i][j], buffer);
				}
			}
		}
		if (!MulRegisterClass(hInst)) return FALSE;
	...
	case WM_COMMAND:
	{
		if (lParam == (LPARAM)btnStart) {
			...
			int FalgGO = AlgrtmDekstr(fromA, toB, n);
			if (FalgGO == 0) {
				...
				wchar_t buffer[256];
				wsprintfW(buffer, L"%d", FalgGO);
				SetWindowTextW(etReslt, buffer);
			}
			return 0;
		}
		break;
			...
			if (n > 0) {
				...
				for (int i = 1; i < n; i++) {
					for (int j = 1; j < n; j++) {
						wchar_t buffer[256];
						wsprintfW(buffer, L"%d", MassMatrix[i][j]);
						SetWindowTextW(etMatrix[i][j], buffer);
					}
				}
			}
			return 0;
...
int AlgrtmDekstr(int AFG, int B,int FGH) {
	int big_num(10000);
...
void saveMatrix() {
	for (int i = 1; i < n; i++) {
		for (int j = 1; j < n; j++) {
			WCHAR value[5];
			GetWindowText(etMatrix[i][j], value, 1000);
			int fg = _wtoi(value);
			if (i == j) 
			{
				MassMatrix[i][j] = 1;
